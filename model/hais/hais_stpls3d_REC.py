...
class ATTBlock(nn.Module):
    def __init__(self, in_channels):
        ...
        self.norm_fn = functools.partial(nn.BatchNorm1d, eps=1e-3, momentum=0.1)

        self.ll = nn.Sequential(
            self.norm_fn(in_channels),
            nn.ReLU(),
    ...
    def forward(self, input):
        # input to maxpolling
        input = self.ll(input)
        input_max = F.normalize(torch.max(input, 0)[0], dim=0)
        output = torch.mul(input, input_max)
        output = output + input
        return output
...
class UBlock(nn.Module):
    def __init__(self, nPlanes, norm_fn, block_reps, block, use_backbone_transformer=True, indice_key_id=1):
