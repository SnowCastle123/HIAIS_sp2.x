...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, xyz, features, batch_ids): # input，xyz = input.coordinates[:,1:4]，features = input.features， batch_ids=input.coordinates[:,0]
        # BND,quchu
        ...
        for i in range(batch_size):
            ...
            if batch_id.size(0) == 0:
                continue
            batch_xyz = xyz[batch_id,:].view(-1, 3)
            batch_features = features[batch_id,:].view(-1, self.d_model)  ### n' * c
